...
/* db에서 유저성향정보 끌어와서 챗봇으로 보내기 */
router.post('/chatbotPush', function (req, res) {    
  console.log('챗봇푸시 라우터 진입 성공')
  var  email = req.body.email
console.log('챗봇푸시라우터 email : '+email)

  //const email = req.body.email
  const sql = `
  select music, drink, hobby, food, name from userinfo where email=?
  `
  conn.query(sql,[email], (err, rows) => {
    
    console.log(rows)
    console.log(rows[0])
    console.log(rows[0].music)
    res.json(
      {
        'name' : rows[0].name,
      'music' : rows[0].music,
